import {basketCalculator} from '../types'
...
export const getBasketTotal = (basket: basketCalculator[]) =>
    basket?.reduce((amount, item) => (item?.price * item?.quantity) + amount, 0);

export const getBasketDiscount = (basket: basketCalculator[]) =>
    basket?.reduce((amount, item) => (item?.discount * item?.quantity*item?.price) + amount, 0);

const reducer = (state: { basket: basketCalculator[]}, action:actions ) => {
